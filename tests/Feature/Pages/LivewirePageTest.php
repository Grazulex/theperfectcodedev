...
test('like componant can be rendered', function (): void {
    $page = makePage();
    $page->status()->publish();
    Livewire::test('pages.like', ['user' => null, 'page_id' => $page->id,'likes_count' => $page->likes->count()])
        ->assertStatus(200)
...
test('like componant can be liked', function (): void {
    ...
    $page = makePage();
    $page->status()->publish();
    Livewire::actingAs($user)->test('pages.like', ['user' => $user, 'page_id' => $page->id,'likes_count' => $page->likes->count()])
        ->assertStatus(200)
        ->assertSee($page->likes->count())
        ->call('like')
...
test('like componant can be followed', function (): void {
    ...
    $page = makePage();
    $page->status()->publish();
    Livewire::actingAs($user)->test('pages.followed', ['user' => $user, 'page_id' => $page->id,'followers_count' => $page->followers->count()])
        ->assertStatus(200)
        ->assertSee($page->likes->count())
        ->call('follow')
