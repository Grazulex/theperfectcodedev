                        ...
                        <div class="col-span-6">
                            ...
                            <div class="flex">
                                @foreach ($this->roles as $index => $role)
                                    <button type="button" class="relative inline-flex last:mr-0 mr-4 border w-full rounded-lg focus:z-10 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 {{ $index > 0 ? 'border-t border-gray-200 dark:border-gray-700 focus:border-none ' : '' }}"
                                                    wire:click="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                                        <div class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }} w-full">
                                            <!-- Role Name -->
                                            <div class="flex items-center justify-between p-4">
                                                <div class="text-2xl font-bold text-gray-400 dark:text-white  {{ $addTeamMemberForm['role'] == $role->key ? 'font-semibold' : '' }}">
                                                    {{ $role->name }}
                                                </div>

                                                @if ($addTeamMemberForm['role'] == $role->key)
                                                    <svg class="w-8 h-8 text-green-400 ms-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            ...
                                            <!-- Role Description -->
                                            <div class="p-4 pb-6 text-sm text-gray-600 dark:text-gray-400 text-start">
                                                {{ $role->description }}
                                            </div>
                                        </div>
                                    </button>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    @if (Gate::check('removeTeamMember', $team))
                                        ...
                                        <button class="text-sm text-red-500 cursor-pointer ms-6 focus:outline-none"
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('updateTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles())
                                        <button class="text-sm text-gray-400 underline ms-2" wire:click="manageRole('{{ $user->id }}')">
                                    ...
                                    @if ($this->user->id === $user->id)
                                        <button class="text-sm text-red-500 cursor-pointer ms-6" wire:click="$toggle('confirmingLeavingTeam')">
                                            {{ __('Leave') }}
                                        </button>

                                    <!-- Remove Team Member -->
                                    @elseif (Gate::check('removeTeamMember', $team))
                                        <button class="text-sm text-red-500 cursor-pointer ms-6" wire:click="confirmTeamMemberRemoval('{{ $user->id }}')">
                                            {{ __('Remove') }}
                                        </button>
                                    @endif
                                </div>
                        ...
                        <div class="{{ $currentRole !== $role->key ? 'opacity-50' : '' }}">
                            ...
                            <div class="flex items-center">
                                ...
                                @if ($currentRole == $role->key)
                                    <svg class="w-5 h-5 text-green-400 ms-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                @endif
                            </div>
