        ...
        <div class="pl-5 pr-5">
            <div class="flex items-center justify-between mb-3">
                <h5 class="block text-xl antialiased font-medium leading-snug tracking-normal text-blue-gray-900">
                    <x-pages.title :title="$page['title']" :published_at="$page['published_at']" />
                </h5>
                v.{{ $page['version'] }}
            </div>
        ...
        <div class="inline-flex mb-3 flex-wrap items-center gap-3 bg-white group dark:bg-[var(--card-dark)] w-full">
            <div class="flex justify-between w-full border-b border-[#3A445B]">
                ...
                <div class="flex self-center">
                    @if (Auth::check())
                        <livewire:pages.followed :user="Auth::user()" :page_id="$page['id']" :followers_count="$page['stats']['followers_count']"/>
                        <livewire:pages.like :user="Auth::user()" :page_id="$page['id']" :likes_count="$page['stats']['likes_count']"/>
                    @else
                        <livewire:pages.followed :user="Null" :page_id="$page['id']" :followers_count="$page['stats']['followers_count']"/>
                        <livewire:pages.like :user="Null" :page_id="$page['id']" :likes_count="$page['stats']['likes_count']"/>
                    @endif
                    <livewire:pages.commented :comments_count="$page['stats']['comments_count']"/>
            ...
            <x-users.card :user="$page['user']" :created_at="$page['user']['created_at']" />
        </div>
        <a href="{{ route('pages.view',['page'=>$page['slug']]) }}"
